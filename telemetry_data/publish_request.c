//publish single telemetry packet and and on off publish according to aws request
...
int main(int argc, char* argv[])
{
	...
	CLOUD gateway_cloud;
        ...
        request_payload.publish_status = 1;
	...
	strcat(gateway_cloud.pub_topic_status,gateway_device.device_ID);
        strcat(gateway_cloud.sub_topic_request,gateway_device.device_ID);
	...
	//subscribing to gateway/request/serial_ID
	if ((rc = MQTTClient_subscribe(client, gateway_cloud.sub_topic_request , QOS)) != MQTTCLIENT_SUCCESS)
	{
		printf("Failed to subscribe, return code %d\n", rc);
		exit(EXIT_FAILURE);
	}
	printf("Subscribed to topic %s for client %s using QoS%d \n", gateway_cloud.sub_topic_request , gateway_device.client_ID, QOS);
	...
	while(1)
	{
                ...
                if(msg_arrvd_flag)
                {      
                        cJSON * jrequest_payload = cJSON_Parse(data_buf);
                        cJSON * jpublish_status = cJSON_GetObjectItem(jrequest_payload,"request");
                        //printf("%s\n",jpublish_status->valuestring);
                        //printf(" : %ld\n",strlen(jpublish_status->valuestring));
                        if(!strcmp(jpublish_status->valuestring, "change_publish_status"))
                        {
                                cJSON * jpublish_status_val = cJSON_GetObjectItem(jrequest_payload,"status");
                                //printf("%s\n",jpublish_status_val->valuestring);
                                if(!strcmp(jpublish_status_val->valuestring, "on"))
                                {
                                        request_payload.publish_status = 1;

                                }
                                else if(!strcmp(jpublish_status_val->valuestring, "off"))
                                {
                                        request_payload.publish_status = 0;
                                }
                        }
		...
		printf("%s\n",cJSON_Print(parsed_json));
                ...
                //sleep(1);
                if(request_payload.publish_status)
                {
